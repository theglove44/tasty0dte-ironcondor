...
monitor.py       - Position P/L monitoring, profit target exits, time exits, EOD expiration
logger.py        - CSV trade logging with schema migration support
local/discord_notify.py - Discord webhook notifications (trade open/close)
local/x_notify.py       - X (Twitter) trade posting
```
...
4. Every 10s: `monitor.check_open_positions()` streams quotes, calculates P/L, triggers exits
