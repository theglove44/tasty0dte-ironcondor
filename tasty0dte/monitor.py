...
async def check_open_positions(session: Session, csv_path: str | None = None, read_only: bool = False):
    """
    ...
    for index, row in open_trades.iterrows():
        try:
            ...
            # Sanity guard: debit can never be negative (short legs are always worth >= long legs).
            # Profit can never exceed the initial credit. Either condition means the quotes are
            # stale or corrupt (e.g. stream dropped mid-cycle). Skip all close decisions.
            if debit_to_close < -0.01 or current_profit > initial_credit + 0.01:
                logger.error(
                    f"Trade {index}: Quote sanity fail — debit={debit_to_close:.2f}, "
                    f"profit={current_profit:.2f} vs credit={initial_credit:.2f}. "
                    f"Stale/corrupt quotes after stream error. Skipping close."
                )
                status_lines.append(
                    f"Trade {index}: BAD QUOTES (debit={debit_to_close:.2f}) — skipped"
                )
                continue
...
async def check_eod_expiration(session: Session, csv_path: str | None = None):
    """
    ...
    for index, row in open_trades.iterrows():
        try:
            ...
            if _is_multi_day(str(strategy_name)):
                ...
                if expiry < today:
                    logger.error(
                        f"JadeLizard trade {index} target_expiry={expiry} was missed "
                        f"(today={today}). Cannot settle with today's SPX close — "
                        f"that price is wrong. Manual CSV correction required."
                    )
                    continue
                # expiry == today: fall through to settlement logic
