<!-- Source: https://developer.tastytrade.com/streaming-account-data/ -->

# Streaming Account Data


- [Getting Started](/streaming-account-data/#getting-started)

- [Available Actions](/streaming-account-data/#available-actions)

- [Receiving Notifications](/streaming-account-data/#receiving-notifications)

- [Notification Nuances](/streaming-account-data/#notification-nuances)

- [Hosts](/streaming-account-data/#hosts)

- [Demo](/streaming-account-data/#demo)
...
You'll notice the status is `Routed`, which means our system is in the process of sending your order to the exchange (for more info on order statuses and their many meanings, see the [Order Flow](/order-flow) page). Since this is a market order, it probably filled immediately. Let's re-fetch this order via the `/GET /accounts/{account_number}/orders/{id}` endpoint to verify:
...
You need to send an `auth-token` with every request made to the websocket server. This is your tastytrade access token. It's the same value you provide in the `Authorization` header of all other API requests. See our [Auth Patterns](/api-overview/#auth-patterns) section for more info.


**NOTE:** When using Access Tokens make sure you have 'Bearer ' preceding your token value within the `auth-token` field.


### 2. Subscribe to Notifications


There are several different types of notifications you can subscribe to. We refer to these types as "actions". You can think of it as subscribing to notifications regarding any actions that occur, such as quote changes, balance changes, watchlist updates, etc. For a complete list of the various actions, see [Available Actions](/streaming-account-data/#available-actions) below.
...
We have a demo page where you can subscribe to your sandbox account data. Head [here](/account-streamer-demo/) to try it out.